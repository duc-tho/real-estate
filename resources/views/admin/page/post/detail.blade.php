<form enctype="multipart/form-data" id="main"
    action="{{ Request::is('admin/post/add') ? route('adminPostPostAdd') : route('adminPostPutEdit', $data['post_info']->PostId) }}"
    method="post">
            ...
            <div class="tab-content">
                <div class="tab-pane active container" id="basicInfo">
                        ...
                        <div class="w-100">
                            <select class="form-control" name="Type" id="Type">
                                ...
                                <option value="bán" {{ ($data['post_info']->Type ?? '') === 'bán' ? 'selected' : '' }}>
                                    Bán</option>
                                <option value="thuê"
                                    {{ ($data['post_info']->Type ?? '') === 'thuê' ? 'selected' : '' }}>Thuê</option>
                            </select>
                        </div>
                        <div class="col-lg-12 messages text-danger"></div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="w-100">
                            <input id="Title" class="form-control" type="text" placeholder="Tiêu đề" name="Title"
                                value="{{ $data['post_info']->Title ?? old('Title') }}">
                        </div>
                        <div class="col-lg-12 messages text-danger"></div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="w-100">
                            <input id="Slug" class="form-control" type="text" placeholder="Slug" name="Slug"
                                value="{{ old('Slug') }}">
                        </div>
                        <div class="col-lg-12 messages text-danger"></div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="w-100">
                            <input id="Price" min="1" class="form-control" type="number" step="0.01" placeholder="Giá"
                                name="Price" value="{{ $data['post_info']->Price ?? old('Price') }}">
                        </div>
                        <div class="col-lg-12 messages text-danger"></div>
                    </div>
                    <div class="form-group">
                                ...
                                @isset($data['category_list'])
                                    @foreach ($data['category_list'] as $item)
                                        <option value="{{ $item->CategoryId }}"
                                            {{ $item->CategoryId === ($data['post_info']->CategoryId ?? '') ? 'selected' : '' }}>
                                            {{ $item->Name }}</option>
                                    @endforeach
                                @endisset
                            </select>
                                ...
                                @isset($data['project_list'])
                                    @foreach ($data['project_list'] as $item)
                                        <option value="{{ $item->ProjectId }}"
                                            {{ $item->ProjectId === ($data['post_info']->ProjectId ?? '') ? 'selected' : '' }}>
                                            {{ $item->Title }}</option>
                                    @endforeach
                                @endisset
                            </select>
                                    ...
                                    @isset($data['city_list'])
                                        @foreach ($data['city_list'] as $item)
                                            <option value="{{ $item->CityId }}"
                                                {{ $item->CityId === ($data['post_info']->CityId ?? '') && ($data['post_info']->ProjectId ?? '') === '' ? 'selected' : '' }}>
                                                {{ $item->Name }}</option>
                                        @endforeach
                                    @endisset
                                </select>
                                        ...
                                        @isset($data['district_list'])
                                            @foreach ($data['district_list'] as $item)
                                                <option value="{{ $item->DistrictId }}"
                                                    {{ $item->DistrictId === ($data['post_info']->DistrictId ?? '') ? 'selected' : '' }}>
                                                    {{ $item->Name }}</option>
                                            @endforeach
                                        @endisset
                                    @endif
                                        ...
                                        @isset($data['area_list'])
                                            @foreach ($data['area_list'] as $item)
                                                <option value="{{ $item->AreaId }}"
                                                    {{ $item->AreaId === ($data['post_info']->AreaId ?? '') ? 'selected' : '' }}>
                                                    {{ $item->Name }}</option>
                                            @endforeach
                                        @endisset
                                    @endif
                                    ...
                                    @if (($data['post_info']->ProjectId ?? '') === '')
                                        @isset($data['street_list'])
                                            @foreach ($data['street_list'] as $item)
                                                <option value="{{ $item->StreetId }}"
                                                    {{ $item->StreetId === ($data['post_info']->StreetId ?? '') ? 'selected' : '' }}>
                                                    {{ $item->Name }}</option>
                                            @endforeach
                                        @endisset
                                    @endif
                ...
                <div class="tab-pane container" id="detailInfo">
                    <div class="form-group">
                        <label class="col-sm-2" for="Apartmentnumber" step="0.01" style="padding-top: 7px;">Số
                            Nhà</label>
                        <div class="w-100">
                            <input id="Apartmentnumber" step="0.01" class="form-control" type="text"
                                placeholder="Số Nhà" name="ApartmentNumber" step="0.01"
                                value="{{ $data['post_info']->ApartmentNumber ?? old('ApartmentNumber') }}">
                        </div>
                        <div class="col-lg-12 messages text-danger"></div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <div class="w-100">
                            <input id="Bedroom" min="0" class="form-control" type="number" step="0.01"
                                placeholder="Số Phòng Ngủ" name="Bedroom"
                                value="{{ $data['post_info']->Bedroom ?? old('Bedroom') }}">
                        </div>
                        <div class="col-lg-12 messages text-danger"></div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="w-100">
                            <input id="Bathroom" min="0" class="form-control" type="number" step="0.01"
                                placeholder="Số Phòng Tắm" name="Bathroom"
                                value="{{ $data['post_info']->Bathroom ?? old('Bathroom') }}">
                        </div>
                        <div class="col-lg-12 messages text-danger"></div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="w-100">
                            <input id="Floor" class="form-control" type="text" placeholder="1 tầng hầm 2 lầu ...."
                                name="Floor" value="{{ $data['post_info']->Floor ?? old('Floor') }}">
                        </div>
                        <div class="col-lg-12 messages text-danger"></div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="w-100">
                            <input id="Length" min="0" class="form-control" type="number" step="0.01"
                                placeholder="Chiều Dài" name="Length"
                                value="{{ $data['post_info']->Length ?? old('Length') }}">
                        </div>
                        <div class="col-lg-12 messages text-danger"></div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="w-100">
                            <input id="Width" min="0" class="form-control" type="number" step="0.01"
                                placeholder="Chiều Rộng" name="Width"
                                value="{{ $data['post_info']->Width ?? old('Width') }}">
                        </div>
                        <div class="col-lg-12 messages text-danger"></div>
                    </div>
                </div>
                                            ...
                                            @if (explode('|', $data['post_info']->Image)[$i] !== '')
                                                <figure class="col-md-4">
                                                    <div class="w-100" style="height: 300px; position: relative;">
                                                        <img alt="picture"
                                                            src="{{ asset(explode('|', $data['post_info']->Image)[$i]) }}"
